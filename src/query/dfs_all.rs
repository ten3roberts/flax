...
/// Visit entities in dfs_alllogical order following `relation`.
///
...
impl DfsAll {
    /// Iterate a hierarchy in dfs_alllogical order from `root`
...
impl<'w, Q, F> QueryStrategy<'w, Q, F> for DfsAll
where
    Q: 'w + Fetch<'w>,
    F: 'w + Fetch<'w>,
{
    type Borrow = DfsAllBorrow<'w, Q, F>;
...
/// Borrowed state for [`DfsAll`] strategy
pub struct DfsAllBorrow<'w, Q, F>
where
    Q: Fetch<'w>,
    F: Fetch<'w>,
{
    ...
    /// Archetypes are in dfs_alllogical order
    prepared: SmallVec<[PreparedArchetype<'w, Q::Prepared, F::Prepared>; 8]>,
}

impl<'w, Q, F> DfsAllBorrow<'w, Q, F>
where
    Q: Fetch<'w>,
    F: Fetch<'w>,
{
    /// Iterate all items matched by query and filter.
    pub fn iter<'q>(&'q mut self) -> DfsAllIter<'w, 'q, Q, F> {
        if self.prepared.is_empty() {
            self.prepared = self
                ...
                .iter()
                .flat_map(|&idx| {
                    let arch_id = self.dfs_all.archetypes[idx];
...
/// Iterates a hierarchy in dfs_alllogical order.
pub struct DfsAllIter<'w, 'q, Q, F>
where
    Q: Fetch<'w>,
    F: Fetch<'w>,
    'w: 'q,
...
impl<'w, 'q, Q, F> Iterator for DfsAllIter<'w, 'q, Q, F>
where
    Q: Fetch<'w>,
    F: Fetch<'w>,
    'w: 'q,
...
mod test {
    ...
    fn dfs_all_query() {
        ...
        let mut query = Query::new(name().cloned())
            ...
            .with(tree())
            .with_strategy(DfsAll::new(child_of));
