...
use syn::{
    ...
    spanned::Spanned,
    token::Paren,
    Attribute, DataStruct, DeriveInput, Error, Expr, Ident, Path, Result, Token,
};
...
fn derive_data_struct(
    ...
    match data.fields {
        syn::Fields::Named(ref fields) => {
            ...
            let impl_generics = quote!(#(#generics: Fetch<'w>),*);
            let impl_generics_prepared = quote!(#(#generics: PreparedFetch<'q>),*);
            ...
            let fetch_struct = quote! {
                impl<#lifetime> #name<#lifetime> {
                    ...
                    pub fn as_fetch(&self) -> impl Fetch {
                        ...
                        impl<'w, #(#generics,)*> #crate_name::Fetch<'w> for #fetch_name<#(#generics),*>
                            where
                                #(#generics: Fetch, <#generics as #crate_name::Fetch>::Prepared: for<'x> #crate_name::PreparedFetch<'x, Item = #types>),*
                        {
                        ...
                        #[automatically_derived]
                        impl<'q, #(#generics,)*> #crate_name::PreparedFetch<'q> for #fetch_name<#(#generics),*>
                            where
